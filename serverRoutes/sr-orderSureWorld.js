...
router.get('/orderSureWorld', function (req, res) {
    var Cookies = renderUtil.splitCookie(req.headers.cookie);
    if(Cookies.uid===''||Cookies.uid===undefined){
        res.cookie('loginFrom','/orderSureWorld');
        res.redirect('/login');
    }else{
        Promise.all([sfFile.read('html/orderSureWorld.html', 'utf8')]).then(onFulfilled, onRejected);
