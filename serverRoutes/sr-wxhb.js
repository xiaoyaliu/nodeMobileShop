...
router.get('/wxhb/:upid', function (req, res) {
    ...
    var modelParam = {
        action: 'getPacketRecInfo',
        user_id:uid,
        packet_id:pid
    };

    httpRequest.get({url: url, formData: modelParam, method: 'get'}).then(function (data) {
        Promise.all([sfFile.read('html/wxhb.html', 'utf8')]).then(onFulfilled, onRejected);
        function onFulfilled(files) {
            try {
                ...
                //*********渲染主体-start***************
                 if(obj.flag){
                     $('[data-attach-point=img]').attr('src',obj.data.pictureWX);
                     $('[data-attach-point=unm]').text(obj.data.nameWX);
                     $('[data-attach-point=qian]').text(obj.data.money);
                 }
