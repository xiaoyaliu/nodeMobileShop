...
router.get('/cartIsWorld', function(req, res) {
    //判断设备
    ...
    if(req.headers.cookie!==undefined){
        ...
        var url=global.dataHost+'/worldwide.php';
        var modelParam = {
            action: 'show_cart',
            sid:Cookies.sid,
            user_id:Cookies.uid
       ...
       httpRequest.get({url: url,formData:modelParam, method: 'get'}).then(function (data) {
           console.log(data);
           Promise.all([sfFile.read('html/cartIsWorld.html', 'utf8')]).then(onFulfilled,onRejected);
               ...
               try{
                   ...
                   //*********渲染主体-start***************
                    if(dataList.info.allcart.length===0){
                        ...
                        $('[data-attach-point=lord]').html(global.dots.cartIsWorld(dataList));
