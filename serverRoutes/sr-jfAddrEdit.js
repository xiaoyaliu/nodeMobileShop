...
router.get('/jfAddrEdit/:id', function (req, res) {
    var str=req.params.id;
    var gid=str.split("_")[1];
    //console.log(gid);
	  var Cookies = renderUtil.splitCookie(req.headers.cookie);
	  if(Cookies.uid===''||Cookies.uid===undefined){
			res.redirect('/login');
	  }else{
			Promise.all([sfFile.read('html/jfAddrEdit.html', 'utf8')]).then(onFulfilled, onRejected);
			function onFulfilled(files) {
				  try {
						...
						$("[data-attach-point='hidGood_id']").val(gid);
